...
| 备料 | 无 | +15%,按(型号 × 砖类型)逐项向上取整:砖 1633、卡扣 1254 |
| 汇总页 | 一张固定的表 | 分组按钮,砖按形状 / 砖类型 / 板号,卡扣按型号 / 砖类型 / 板号 |
| 卡扣颜色 | 全部同一个蓝 / 同一种钢 | 一种一个颜色,出处 `data/clip_colours.json`,贴图 / DXF / 模型 / 网页 / 照片同色 |
| 图纸可重现 | SVG 带时间戳、DXF 带创建时间,每次生成字节都不同 | SVG 固定 hashsalt 且不写日期;DXF 时间与 GUID 钉死;下载区按**图的内容**比对而非字节 |
