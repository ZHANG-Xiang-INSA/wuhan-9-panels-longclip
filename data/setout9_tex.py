...
def bake(i):
    ...
    for lc in B['longs']:
        ...
        ax.text(lc['clab'][0], lc['clab'][1], lc['ccode'], ha='center', va='center',
                fontsize=lc['th']*PXMM*72.0/100.0, color=CLIP, zorder=6)
    for p in B['pieces']:
        ax.add_patch(Polygon(p['slip'], closed=True, fill=False, ec=INK, lw=lw, zorder=2))
        if p['tray']:
            ax.add_patch(Polygon(p['tray'], closed=True, fill=False, ec=CLIP, lw=lw*0.85,
                                 zorder=3))
        ...
        if p['ccode']:
            ax.text(p['clab'][0], p['clab'][1], p['ccode'], ha='center', va='center',
                    fontsize=fs, color=CLIP, zorder=6)
